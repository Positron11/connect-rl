...
class CXEnvironment:
	...
	def __str__(self) -> str:
		...
		return "\n".join((" ".join(visual[i] for i in row)) for row in np.flip(self.board.T, 0))
	...
	def get_free(self) -> list:
		...
		indices = [np.where(c == 0)[0] for c in self.board]
		return [int(i[0]) if len(i) else -1 for i in indices]
	...
	def board_full(self) -> bool:
		...
		return all([i for i in self.board.T[-1]])
	...
	def play(self, col) -> tuple[int, bool]:
		...
		else: self.board[col][row] = self.current_player

		win, block = self.check_win(row, col)
		done = win or self.board_full()

		# switch player
		self.current_player = 3 - self.current_player

		return done, win, block
	...
	def check_win(self, row, col) -> tuple[bool, float]:
		...
		for dr, dc in directions:
			...
			# positive direction.
			r, c = row + dr, col + dc
			while self.is_valid(r, c) and self.board[c, r] == player:
				player_count += 1
				r += dr
				c += dc
		...
		for dr, dc in directions:
			...
			# positive direction.
			r, c = row + dr, col + dc
			while self.is_valid(r, c) and self.board[c, r] == opp:
				opp_count += 1
				r += dr
				c += dc
		...
		if single_partial_blocks > 1: block_reward_weight += single_partial_blocks

		return False, block_reward_weight
	

	def canonicalized(self) -> tuple[int, tuple]:
		"""Return the canonical form of the board."""

		# mirror board along vertical axis
		mirrored = np.flipud(self.board)

		# return canonical form as per lexicographic ordering
		return 3 - self.current_player, max(tuple(self.board.flatten()), tuple(mirrored.flatten()))
